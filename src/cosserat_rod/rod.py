...
class Rod(ABC):
    ...
    def _init_model_parameters(self):
        ...
        if type(self.model_parameters.B_tilde) == np.ndarray:
            self.B_tilde = Constant(self.model_parameters.B_tilde)
        elif type(self.model_parameters.B_tilde) == Expression:
            self.B_tilde = self.model_parameters.B_tilde
    ...
    # These need to be overloaded by the different rod classes
    @abstractmethod
    ...
    @abstractmethod   
    def _get_initial_functions(self, F0, proj = False):
        pass
    ...
    @abstractmethod    
    def _init_weak_form_simple(self):
        pass 
    
    @abstractmethod    
    def _init_weak_form_picard(self):
        pass 
    ...
    def _init_centerline(self):
        ...
        x0  = project(x0, self.function_spaces['x'])
    ...
    def _init_angular_velocity(self):
        ...
        w0 = Function(self.function_spaces['w'])
        w0.assign(w0_expr)
        
        return w0

    def _init_generalized_curvature(self):
                
        Ome0_expr = Expression(('0', '0', '0'), degree = 1)
        Ome0 = Function(self.function_spaces['Omega'])
        Ome0.assign(Ome0_expr) 
        
        return Ome0

    def _assign_initial_functions(self, F0, proj = False):

        # Assign local frame vectors
        self.e1.assign(F0.e1)
        self.e2.assign(F0.e2)
        self.e3.assign(F0.e3)
        
        # Assign forces and strains
        fa = FunctionAssigner(self.W, self.VV)
        
        function_list = self._get_initial_functions(F0, proj)
        
        fa.assign(self.u_n, function_list) 
                           
        function_list = self._get_initial_functions(F0, proj)
            
    def _init_weak_form(self):
        
        if self.solver.linearization_method == 'simple':
            self._init_weak_form_simple()
        elif self.solver.linearization_method == 'picard_iteration':
            self._init_weak_form_picard()
        elif self.solver.linearization_method == 'newton':
            pass                            
        return

    def _update_local_reference_frame(self, w):
    ...
    def solve(self, T, CS, F0 = None):
        ...
        if F0 is None:            
            F0 = self._init_frame()
            self._assign_initial_functions(F0)        
        else:
            self._assign_initial_functions(F0, proj= True)        
        ...
        for C in CS:
            ...
            self._update_local_reference_frame(w)            
...
class CosseratRod(Rod):
    
    def __init__(self, N, dt, model_parameters = None, solver = None):
        
        self.rod_type = 'Cosserat'
    
        super().__init__(N, dt, model_parameters, solver)
                
    def _init_model_parameters(self):
        
        Rod._init_model_parameters(self)
                
        # Material parameters for shear and stretch            
        ...
        if type(self.model_parameters.S_tilde) == np.ndarray:
            self.S_tilde = Constant(self.model_parameters.S_tilde)
        elif type(self.model_parameters.S_tilde) == Expression:
            self.S_tilde = self.model_parameters.S_tilde
    ...
    def _init_function_space(self):
        ...
        self.function_spaces['M'] = self.V3
        self.function_spaces['F'] = self.V3
        
        self.function_spaces['x'] = self.V3
        self.function_spaces['Omega'] = self.V3
        self.function_spaces['sigma'] = self.V3
        self.function_spaces['w'] = self.V3        
    ...
    def _init_controls(self):
        ...
        self.Ome_pref = Function(self.function_spaces['Omega'])
        self.sig_pref = Function(self.function_spaces['sigma'])
                        
        return

    def _get_initial_functions(self, F0, proj):

        F = Function(self.function_spaces['F']) 
        M = Function(self.function_spaces['M'])
    ...
    def _init_frame(self):
        ...
        Ome0 = self._init_generalized_curvature()
    ...
    def _init_weak_form_simple(self):
        ...
        _, _, x_n, Ome_n, sig_n, _ = split(self.u_n)
        ...
        eq_F = dot(F, phi_F) * dx - dot(self.S * (sig - self.sig_pref), phi_F) * dx - dot(self.S_tilde * (sig - sig_n) / self.dt, phi_F) * dx
        eq_M = dot(M, phi_M) * dx - dot(self.B * (Ome - self.Ome_pref), phi_M) * dx - dot(self.B_tilde * (Ome - Ome_n) / self.dt, phi_M) * dx
    ...
    def _init_weak_form_picard(self):
        ...
        eq_F = dot(F, phi_F) * dx - dot(self.S * (sig - self.sig_pref), phi_F) * dx - dot(self.S_tilde * (sig - self.sig_tilde_n) / self.dt, phi_F) * dx
        eq_M = dot(M, phi_M) * dx - dot(self.B * (Ome - self.Ome_pref), phi_M) * dx - dot(self.B_tilde * (Ome - self.Ome_tilde_n) / self.dt, phi_M) * dx
    ...
    def _update_frame(self):
        ...
        _, _, x, Ome, sig, w =  self.u_n.split()        
...
class InextensibleKirchhoffRod(Rod):
    
    def __init__(self, N, dt, model_parameters, solver):

        self.rod_type = 'inextensible_Kirchhoff'
    ...
    def _init_function_space(self):
        ...
        #F, M, x, Omega, w         
        self.VV = [self.V3, self.V3, self.V3, self.V3, self.V3]         
        
        #F, M, x, Omega, sigma, w
        self.W = FunctionSpace(self.mesh, MixedElement(5 * [P1_3]))
    ...
    def _init_weak_form_simple(self):
        ...
        # Solution from previous time step        
        _, _, x, Ome_n, _ = split(self.u_n)[3]
        
        xu_n = self.xu_0 # Length element should not change
        ...
        F, M, x, Ome, w = split(u)
        phi_F, phi_M, phi_x, phi_Ome, phi_w = split(v)
        
        # External force
        if self.model_parameters.external_force == 'linear_drag':            
            KK = self.K         
        elif self.model_parameters.external_force == 'resistive_force':             
            e3e3 = outer(self.e3, self.e3)
            P = Identity(3) - e3e3            
            KK = self.K*P + e3e3
        ...
        F = -cross(self.e3, self.K_rot * w) + cross(self.e3, grad(M)) / xu_n + p*self.e3 
        ...
        #eq_Ome = dot(-self.Q * self.K_rot * w, phi_Ome) * xu_n * dx + dot(cross(tau_n, self.Q * F), phi_Ome) * xu_n * dx - dot(self.Q * M, grad(phi_Ome)) * dx  
        eq_Ome = -dot(self.E, self.K_rot * w) * phi_Ome * xu_n * dx + dot(self.E3, cross(Ome_n, M)) * phi_Ome * xu_n * dx - dot(self.E3, M) * grad(phi_Ome) * dx          
        
        eq_w = dot(grad(w) / xu_n - (Ome - self.Ome_n) / self.dt - cross(w, Ome_n), phi_w) * dx 
        
        eq_F = dot(self.E3 - self.Q.T * grad(x)/ self.xu_0, phi_F) * dx
        
        EQ = eq_F + eq_M + eq_x + eq_Ome + eq_w 
    ...
    def _init_weak_form_picard(self):
        ...
        # Update approximation of functions at the current time step to be the solution 
        # of the last iteration step in the picard iteration
        _, _, x_n, Ome_n, _ = split(self.u_n)
        ...
        #eq_F = dot(KK * (x - self.x_tilde_n), phi_x) / self.dt * xu_n * dx - dot(self.Q * F, grad(phi_x)) * dx        
        eq_F = dot(KK * (x - self.x_tilde_n), phi_F) / self.dt * xu_n * dx - dot(F, grad(phi_F)) * dx        
        ...
        eq_x = dot(self.E3 - self.Q.T * grad(x)/ self.xu_0, phi_x) * dx
    ...
    def _update_frame(self):
        ...
        _, _, x, Ome, w =  self.u_n.split()        
        self.F.update(x, self.e1, self.e2, self.e3, Ome, w, t = self.t)
    ...
    def _solve_picard_iteration(self):
        ...
        # Update functions from the previous time which are used in the discretized version 
        # of the time derivatives. These will not be updated during picard iteration!         
        _, _, x_tilde_n, Ome_tilde_n, _ = split(self.u_n)
...
class ExtensbileKirchhoffRod(Rod):
    
    def __init__(self, N, dt, model_parameters, solver):
        
        self.rod_type = 'extensible_Kirchhoff'
